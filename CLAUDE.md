...
- **QR generato nel browser puntando al dominio canonico** (`SITE_URL` in `lib/site.ts`, hardcoded a `albeaconcierge.it`, override raro via `NEXT_PUBLIC_SITE_URL`): mai `window.location.origin`, così un QR generato da una preview Vercel o da localhost stampa comunque l'indirizzo giusto (fix 2026-07-10, review Codex). Colori fissi nero/bianco (un QR a basso contrasto non si scansiona).
