...
2. 🟡 **MITIGATO (2026-07-10), non chiuso — Feedback ospite senza limite.** Applicata la seconda barriera (a livello applicativo); manca la prima barriera al margine della rete. Cosa c'è:
   ...
   - Tetto di memoria rigido (`MAX_BUCKETS = 10_000` con sfratto dei bucket più vicini a scadere): senza, un flood di IP DISTINTI nella stessa finestra farebbe crescere la Map senza limiti (i bucket vivi non vengono spazzati). Ora la memoria resta limitata comunque.
   - Test automatico in CI (`npm run test:unit`, `scripts/test-rate-limit.mjs`): limite, indipendenza delle chiavi, scadenza della finestra e tenuta del tetto sotto flood di 20k chiavi. Non più solo verifica manuale.
   - **Limite strutturale che resta aperto:** il contatore è per-istanza serverless e si azzera ai cold start; un abuso DISTRIBUITO su molti IP può aggirarlo continuando a generare costi. La difesa vera contro quello va messa al margine della rete, PRIMA che la funzione parta: rate limit / WAF di Vercel (dashboard, oppure `@vercel/firewall` per regole programmatiche). Finché non c'è quello, questo punto è mitigato per il pilota ma non definitivamente risolto. **TODO prima di aprire il QR al pubblico su larga scala:** configurare il rate limit di Vercel sull'endpoint del feedback.
