...
2. ✅ **RISOLTO (2026-07-10) — Feedback ospite senza limite.** `src/lib/rate-limit.ts`: finestra fissa in-memory (`isRateLimited(key, limit, windowMs)`), applicata in `submitGuestFeedback` su due chiavi — per IP+struttura (5 invii / 10 minuti, generoso per un ospite vero) e per struttura intera (30 invii / ora, tetto sui costi anche con IP diversi/bot distribuito). Aggiunto anche un campo honeypot invisibile (`website`, `sr-only` + `aria-hidden` + fuori dal tab order) in `review-module.tsx`: se arriva compilato si finge successo senza scrivere, senza dare indizi al bot. In ogni caso di blocco il messaggio resta il generico "qualcosa è andato storto" (mai un errore specifico che riveli la causa). Limite noto e accettato: il contatore vive nella memoria del processo, non è distribuito tra istanze serverless e si azzera ai cold start — adeguato alla scala di un pilota, da rivedere (Upstash/Redis) se il traffico crescesse molto. Verificato con test isolato (Node, non serve rete/Supabase): 5 richieste passano, la 6ª e la 7ª bloccate, chiavi diverse indipendenti, contatore che riparte dopo la scadenza della finestra; verificato anche a mano nel browser che un invio reale continua a funzionare e il campo honeypot resta invisibile e fuori dall'albero di accessibilità.
