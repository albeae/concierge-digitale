...
2. ✅ **RISOLTO (2026-07-10) — Feedback ospite senza limite.** Entrambe le barriere ora attive: applicativa + rete. Cosa c'è:
   ...
   - **Barriera di rete (WAF Vercel) pubblicata (2026-07-10, confermato dal proprietario):** regola sul piano Hobby, Fixed Window, POST alle route guest (esclusa `/admin`), 10 req/60s per IP, Deny 403 — specifica completa in `docs/vercel-waf-feedback-rate-limit.md`. Ferma un abuso distribuito su molti IP PRIMA che la funzione parta, colmando il limite del contatore in-app (per-istanza, si azzera ai cold start). Nota di scalabilità futura: lo sfratto LRU in-app resta ottimo per il pilota; per flood enormi in-process valutare una coda/min-heap dedicata — ma col WAF davanti non dovrebbe mai essere il collo di bottiglia.
3. ✅ **RISOLTO (2026-07-10) — Riordino posti non atomico.** Il riordino ora è una funzione SQL, `public.reorder_places(bnb_id, ordered_ids[])` (`supabase/reorder-places.sql`), chiamata via `supabase.rpc` da `reorderPlaces`. Prima di scrivere valida che la lista sia ESATTAMENTE una permutazione dei posti della struttura (stessa cardinalità, nessun duplicato, ogni id appartiene alla struttura) e poi riscrive `sort_order = posizione` in **un'unica UPDATE** dentro la funzione (atomica: o tutte le righe o nessuna). Una richiesta parziale/duplicata/manipolata solleva un'eccezione → nessuna scrittura parziale. `security invoker` (la RLS resta valida) + controllo esplicito di proprietà. Due rifiniture dopo una seconda review: (a) `perform ... for update` sulla riga di `bnb_clients` PRIMA di contare i posti, non solo per il controllo di proprietà — serializza due riordini concorrenti sulla stessa struttura e, grazie al lock che il vincolo FK prende sulla riga padre, blocca anche un insert concorrente di un nuovo posto finché la transazione non commit; (b) `revoke execute ... from public` esplicito (Postgres concede EXECUTE a PUBLIC di default su ogni funzione nuova, il solo `grant ... to authenticated` non lo toglie — non era una falla diretta, visto il controllo owner_id, ma superficie inutile). Test in `test:sql`: permutazione valida, lista parziale (con verifica che l'ordine NON cambi), duplicato, id estraneo, non-proprietario, EXECUTE revocato a PUBLIC/concesso ad authenticated, anon non può chiamarla. **⚠️ Migrazione da applicare in Supabase PRIMA del deploy** (vedi checklist migrazioni) — se l'avevi già applicata prima di questo hardening, rieseguila: è idempotente.
...
5. ✅ **RISOLTO (2026-07-10) — CI dopo il deploy.** Trigger `pull_request` aggiunto al workflow (`on: [push, pull_request]`) e **branch protection attivata su GitHub** (ruleset su `main`: "Require a pull request before merging" + "Require status checks to pass" col check `verifica`, nessun bypass). Da questo momento `main` non è più pushabile direttamente: ogni modifica passa da branch + PR con CI verde prima del merge. Il flusso di lavoro di questo repo cambia di conseguenza (vedi sezione 6).
