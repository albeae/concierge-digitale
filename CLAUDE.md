...
2. 🟡 **MITIGATO (2026-07-10), non chiuso — Feedback ospite senza limite.** Applicata la seconda barriera (a livello applicativo); manca la prima barriera al margine della rete. Cosa c'è:
   ...
   - **Limite strutturale che resta aperto:** il contatore è per-istanza serverless e si azzera ai cold start; un abuso DISTRIBUITO su molti IP può aggirarlo continuando a generare costi. La difesa vera contro quello va messa al margine della rete, PRIMA che la funzione parta: rate limit / WAF di Vercel. **Il WAF è incluso anche nel piano gratuito**, quindi è realizzabile subito senza upgrade. **TODO prima di aprire il QR al pubblico su larga scala:** configurare una regola di rate limit nel WAF Vercel sul path del feedback (dashboard → Firewall, oppure `@vercel/firewall` per regole versionate nel repo). Nota di scalabilità futura: lo sfratto LRU attuale è ottimo per il pilota; se un giorno servisse gestire flood enormi in-process, valutare una struttura a coda/min-heap dedicata — ma con il WAF davanti non dovrebbe mai essere il collo di bottiglia.
